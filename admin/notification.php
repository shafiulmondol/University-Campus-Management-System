...
function getUnreadCounts($con, $admin_email) {
    ...
    $sections = ['Student', 'Staff', 'Department', 'Faculty', 'Library', 'Account', 'Admin', 'Bank'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_notice'])) {
    ...
    if ($stmt = $con->prepare("INSERT INTO notice (id, title, section, content, author, created_at, viewed) VALUES (?, ?, ?, ?, ?, NOW(), 0)")) {
        $stmt->bind_param("issss", $id, $title, $section_notice, $content, $author);
        if ($stmt->execute()) {
...
<head>
    ...
    <style>
        ...
        .filter-form select, .filter-form input[type="text"]{ padding:10px 12px; border:1px solid var(--border); border-radius:10px; min-width:200px; }
        ...
        /* Notice cards */
        .notice-card{ background:#fff; border:1px solid var(--border); border-radius:12px; padding:14px; margin-bottom:12px; transition:background 0.2s; }
    ...
    <div class="panel">
        ...
        <form method="post">
            <div class="add-form-grid">
                ...
                <div>
                    <label>Section</label>
                    <select name="section_notice" required>
                        <?php
                        $sections_notice = ['Student', 'Staff', 'Department', 'Faculty', 'Library', 'Account', 'Admin', 'Bank'];
...
<div class="panel">
    ...
    <form method="post" class="filter-form">
        ...
        <select name="section" id="section">
            <?php
            $sections = ['Student', 'Faculty', 'Staff', 'Admin', 'AO', 'Bank', 'update_request'];
    ...
    $notices = [];
    if ($selected_section !== 'update_request') {
        $sec_safe = mysqli_real_escape_string($con, $selected_section);
        $query1 = "SELECT id, title, content, author, created_at, viewed, section FROM notice WHERE section='{$sec_safe}' ORDER BY created_at DESC";
            ...
            if ($row['source'] === 'notice') {
                ...
                <div class="<?php echo $noticeClass; ?>">
                    <div class="notice-header">
                        ...
                        <span class="notice-section"><?php echo htmlspecialchars($row['section']); ?></span>
                    </div>
    ...
    // Remove the automatic marking of Admin notices as read
    // if ($selected_section === 'Admin') {
    //     mysqli_query($con, "UPDATE notice SET viewed = 1 WHERE viewed = 0 AND section='Admin'");
    // }
    ?>
</div>
<?php ob_end_flush(); ?>